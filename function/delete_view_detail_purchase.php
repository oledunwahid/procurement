...
if (isset($_POST['id'])) {
    ...
    try {
        ...
        $oldDataQuery = "SELECT rd.*, pr.title as request_title 
                        FROM proc_request_details rd
                        LEFT JOIN proc_purchase_requests pr ON rd.id_proc_ch = pr.id_proc_ch 
                        WHERE rd.id = ?";
        $stmtOld = mysqli_prepare($koneksi, $oldDataQuery);
        mysqli_stmt_bind_param($stmtOld, "i", $id);
        mysqli_stmt_execute($stmtOld);
        $oldData = mysqli_stmt_get_result($stmtOld)->fetch_assoc();
        ...
        $oldValue = json_encode([
            ...
            'request_title' => $oldData['request_title'],
